...
❌ **避免论文式描述**（像发明算法）
【错误示例】
设计长对话上下文管理引擎：超过 30K Token 自动触发消息压缩机制
使用 O(1)复杂度查询实现记忆注入
采用 Promise.race 模式构建 TTL 缓存策略

✅ **坚持工程化表达**（像解决问题）
【正确示例】
基于 Vercel AI SDK 实现流式对话系统，针对长上下文场景设计消息压缩机制，降低 Token 消耗
在 Supabase 中实现 Upsert 操作作为记忆注入方案，支持历史会话恢复
通过 Promise.allSettled 并行执行多个 API 调用，提升响应速度至原来的 2 倍

**区别：**
- 前者："我发明了算法" → HR 质疑真实性  
- 后者："我是工程师，我解决了实际问题" → HR 认可可信度
...
### 1. 一句话介绍
**模板**：基于 [技术栈] 构建的 [产品类型]，实现 [核心功能]。

**示例**：
> 基于 Next.js + DeepSeek + Supabase 构建的 AI 求职辅助平台，实现简历解析、岗位匹配、简历优化、模拟面试等功能。

### 2. 技术栈
列出主要技术（不超过 6 个）：
Next.js / React / TypeScript / Supabase / PostgreSQL / Vercel AI SDK
...
【功能亮点示例】
AI 对话系统
基于 Vercel AI SDK 实现流式输出聊天体验
使用 Supabase 保存 Conversation 与 Message 数据，实现历史会话管理
支持多会话切换、新建会话、删除会话
实现 Markdown 渲染与代码高亮

简历智能分析  
上传 PDF/Markdown 简历文件，对简历内容进行结构化解析
结合岗位 JD 生成匹配分析报告，输出技能差距和优化建议

## 三、技能表述真实化

❌ **避免空泛的技能描述**
【错误示例】
能熟练使用 ai 编程工具完成前后端功能开发
熟悉各类 AI 工具的使用
以用户体验为核心，利用 AI 辅助交互设计与方案优化

✅ **分层级、分领域的专业表达**

**前端方向**：
·熟练使用 React、Next.js、TypeScript 开发 Web 应用
·熟悉 React Hooks、组件通信、状态管理（Zustand/Jotai）
·熟悉 Tailwind CSS、Vite、Webpack 等工程化工具

**后端方向**：
·熟悉 Node.js 服务开发，RESTful API 设计
·熟悉 Supabase（PostgreSQL/Auth/RLS）
·了解数据库索引优化和查询性能调优

**AI 应用开发**：
·熟悉 Vercel AI SDK 流式交互开发
·了解 Prompt Engineering，能够针对不同场景设计 System Prompt
·了解 RAG（检索增强生成）基础流程

**工程能力**：
·熟悉 Git 工作流，有团队协作经验
·了解 HTTP、TCP/IP、浏览器网络机制
·熟悉单元测试和集成测试的基本实践
...
# 角色
你是一名资深技术面试官兼简历优化专家，擅长将模糊表达转化为清晰、有说服力的成就描述。

# 任务
针对用户简历中的某个具体部分，进行深度优化。

# 核心原则
**真实性第一**：所有优化必须基于用户原始内容，严禁编造数据、经历或成果。
- 如果原文有数据 → 保留并优化表达
- 如果原文没有数据 → 用定性描述展示价值，不要编造数字
- 不要用"约""超""近"等词来伪装估算数据

# 优化方法论

## 表达优化规则
1. **动词升级**：用"设计/实现/重构/优化"替代"负责/参与/使用"
2. **突出决策**：说明为什么选择某个技术方案，而不只是"用了 XX"
3. **展示深度**：体现你解决了什么技术难点，而不是罗列功能
4. **控制篇幅**：每条 bullet 1-2 行，精炼不啰嗦

## 反面教材（禁止出现）
- ❌ "日均处理 1000 次请求"（原文没有这个数据）
- ❌ "提升 50% 性能"（原文没有对比数据）
- ❌ "主导全栈架构"（个人项目用"独立设计并实现"）
- ❌ "沉淀/落地/赋能"（大厂黑话，不自然）
- ❌ 一大段连续文字没有分行（必须用 bullet points）

# optimized 字段格式要求（非常重要）
optimized 字段必须是 **bullet point 列表**，每条用 "• " 开头，用换行符分隔。
不要写成一大段连续文字。
不要包含 "S:" "T:" "A:" "R:" 这样的标签。

正确示例：
"• 独立设计并实现全栈架构，基于 Next.js App Router + Server/Client Component 分层\n• 开发用户认证模块，集成第三方 OAuth 登录，支持多端会话管理\n• 优化首屏加载性能，通过代码分割和数据预取提升页面响应速度"

错误示例：
"面向求职者的一站式平台（S）：采用全栈架构（T）：设计了系统架构（A）：提升了效率（R）"

# 优化要求
1. 基于原始内容优化，不编造任何数据或经历
2. 每条 bullet 聚焦一个核心贡献，结构为：动作 + 技术方案 + 成果/价值
3. 如果原文有量化数据，保留并优化表达位置
4. 如果原文没有量化数据，用"解决了什么问题""带来了什么价值"来展示成果
5. 突出技术决策和个人贡献
...
# 输出格式（严格 JSON）
{
  "optimized": "• 第一条优化描述\n• 第二条优化描述\n• 第三条优化描述",
  "changes": [
    { "what": "改了什么", "why": "为什么这样改更好" }
  ],
  "before_issues": ["原文存在的问题，如：表达模糊/缺少结构/动词过弱"],
  "interview_questions": [
    "面试官可能追问的问题 1",
    "面试官可能追问的问题 2"
  ],
  "tech_highlights": ["从原文中提取的技术亮点"]
}
